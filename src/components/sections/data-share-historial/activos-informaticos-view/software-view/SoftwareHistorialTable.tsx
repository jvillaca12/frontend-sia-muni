...
import { GridApi, DataGrid, GridSlots, GridColDef, useGridApiRef } from '@mui/x-data-grid';
import CustomPagination from 'components/common/CustomPagination';
import CustomNoResultsOverlay from 'components/common/CustomNoResultsOverlay';
import { rows } from './historial-software';
...
// props de la tabla de SoftwareHistorialTable
const SoftwareHistorialTable = ({
  searchText,
  onExportPDF,
  onExportExcel,
  onFilteredRowCountChange,
}: {
  // pasando las props de la tabla de SoftwareHistorialTable
  searchText: string;
  onExportPDF: (func: () => void) => void;
  onExportExcel: (func: () => void) => void;
  onFilteredRowCountChange: (count: number) => void;
}): ReactElement => {
  // estado de la tabla de SoftwareHistorialTable
  ...
  // columnas de la tabla de SoftwareHistorialTable
  const columns: GridColDef<any>[] = [
    ...
    },
    {
      ...
      headerName: 'Fecha de Vencimiento de Licencia',
      headerAlign: 'center',
      resizable: false,
      align: 'center',
      minWidth: 350,
    ...
    (type: 'current' | 'filtered' | 'all') => {
      ...
      if (exportType === 'PDF') {
        exportToPDF(
          ...
          'software-reporte',
          type,
          'Reporte de los de los activos informáticos con relación a Software de la MPP',
          user,
          'Reporte de toda la información de los activos informáticos con relación a Software, de acuerdo al registro en tiempo real del sistema',
        );
      } else {
        exportToExcel(apiRef, rows, exportColumns, 'hardware-reporte', type);
      }
...
export default SoftwareHistorialTable;
