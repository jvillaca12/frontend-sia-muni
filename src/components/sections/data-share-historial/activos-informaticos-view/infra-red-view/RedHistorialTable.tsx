...
import { rows } from './historial-red';
...
// props de la tabla de RedHistorialTable
const RedHistorialTable = ({
  searchText,
  onExportPDF,
  onExportExcel,
  onFilteredRowCountChange,
}: {
  // pasando las props de la tabla de RedHistorialTable
  searchText: string;
  onExportPDF: (func: () => void) => void;
  onExportExcel: (func: () => void) => void;
  onFilteredRowCountChange: (count: number) => void;
}): ReactElement => {
  // estado de la tabla de RedHistorialTable
  ...
  // columnas de la tabla de RedHistorialTable
    ...
    (type: 'current' | 'filtered' | 'all') => {
      ...
      if (exportType === 'PDF') {
        exportToPDF(
          ...
          'red-reporte',
          type,
          'Reporte de los de los activos informáticos con relación a Infraestructura de Red de la MPP',
          user,
          'Reporte de toda la información de los activos informáticos con relación a Infraestructura de Red, de acuerdo al registro en tiempo real del sistema',
        );
      } else {
        exportToExcel(apiRef, rows, exportColumns, 'red-reporte', type);
      }
...
export default RedHistorialTable;
