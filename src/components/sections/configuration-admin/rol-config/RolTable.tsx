...
import { dataRolAll } from './data-rol';
import ModalEliminarRol from './ModalEliminarRol';
import FormActualizarRol from './FormActualizarRol';
import { RolDTO } from 'data/interfaces/RolDTO';

// props de la tabla de rol
const RolTable = ({
  searchText,
  onExportPDF,
  onExportExcel,
  ...
  // pasando las props de la tabla de rol
  searchText: string;
  ...
  // estado de la tabla de rol
  const apiRef = useGridApiRef<GridApi>();
  // estado de la fila seleccionada para actualizar
  const [selectedRowUpdate, setSelectedRowUpdate] = useState<RolDTO | null>(null);
  // estado del id de la fila seleccionada para actualizar
  const [selectedIdRol, setSelectedIdRol] = useState<number>(0);
  // estado del modal de actualizar rol
  ...
  const [selectedRow, setSelectedRow] = useState<RolDTO | null>(null);
  ...
  // estado de las filas de la tabla de rol
  const { rows, isLoading } = dataRolAll(onFilteredRowCountChange);
  ...
  const handleOpenModal = (row: any, id: number) => {
    setSelectedRow(row);
    setModalMessage(`Esta seguro de eliminar este rol con el id: ${id}`);
    setOpenModal(true);
    setIsDeleted(false);
  };
    ...
    (type: 'current' | 'filtered' | 'all') => {
      ...
      if (exportType === 'PDF') {
        exportToPDF(
          ...
          'rol-reporte',
          type,
          'Reporte de los Roles asociados al personal',
          user,
          'Reporte de los Roles, de acuerdo al registro del sistema actual',
        );
      } else {
        exportToExcel(apiRef, rows, exportColumns, 'rol-reporte', type);
      }
  ...
  // para abrir el modal de actualizar rol
  ...
  // para cerrar el modal de actualizar rol
