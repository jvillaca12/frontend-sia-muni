...
import FormActualizarPrioridad from './FormActualizarPrioridad';
import { PrioridadDTO } from 'data/interfaces/PrioridadDTO';
import ModalEliminarPrioridad from './ModalDeletePrioridad';
import { dataPrioridadAll } from './data-prioridad';

// props de la tabla de Prioridad
const PrioridadTable = ({
  searchText,
  onExportPDF,
  onExportExcel,
  ...
  // pasando las props de la tabla de Prioridad
  searchText: string;
  ...
  // estado de la tabla de prioridad
  const apiRef = useGridApiRef<GridApi>();
  // estado de la fila seleccionada para actualizar
  const [selectedRowUpdate, setSelectedRowUpdate] = useState<PrioridadDTO | null>(null);
  // estado del id de la fila seleccionada para actualizar
  const [selectedIdPrioridad, setSelectedIdPrioridad] = useState<number>(0);
  // estado del modal de actualizar prioridad
  ...
  const [selectedRow, setSelectedRow] = useState<PrioridadDTO | null>(null);
  ...
  // estado de las filas de la tabla de prioridad
  const { rows, isLoading } = dataPrioridadAll(onFilteredRowCountChange);

  // columnas de la tabla de prioridad
  ...
  const handleOpenModal = (row: any, id: number) => {
    setSelectedRow(row);
    setModalMessage(`Esta seguro de eliminar esta prioridad con el id: ${id}`);
    setOpenModal(true);
    setIsDeleted(false);
  };
    ...
    (type: 'current' | 'filtered' | 'all') => {
      ...
      if (exportType === 'PDF') {
        exportToPDF(
          ...
          'pririodad-reporte',
          type,
          'Reporte de las Prioridades de los Fallos/Incidencias',
          user,
          'Reporte de las Prioridades, de acuerdo al registro del sistema actual',
        );
      } else {
        exportToExcel(apiRef, rows, exportColumns, 'prioridad-reporte', type);
      }
  ...
  // para abrir el modal de actualizar pririodad
  const handleOpenUpdate = (row: any, id: number) => {
    setSelectedRowUpdate(row);
    setSelectedIdPrioridad(id);
    setOpenUpdateModal(true);
  };

  // para cerrar el modal de actualizar prioridad
  const handleCloseUpdate = () => {
    setSelectedRowUpdate(null);
    setSelectedIdPrioridad(0);
  ...
  return (
    <>
      ...
      />
      {/* para actualizar una prioridad */}
      <FormActualizarPrioridad
        openUpdateModal={openUpdateModal}
        onClose={handleCloseUpdate}
        row={selectedRowUpdate}
        id={selectedIdPrioridad}
        onSuccessRegistro={onSuccessRegistro}
      />
      {/* para eliminar una prioridad */}
      <ModalEliminarPrioridad
